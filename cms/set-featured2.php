...
if(isset($_SESSION["personId"]) && isset($_SESSION["userId"])) {
	...
	<html>
    	<head>
    	    <title>Consuper CMS | Set Featured 2</title>
    	    <meta charset="utf-8">
        ...
        <?php include("../includes/2ndlevel-header.php"); ?>
            <main>
                ...
                <h2>Set Featured Product 2</h2>
                <?php include('../includes/db-config.php');
                $stmt = $pdo->prepare("SELECT * FROM `product` WHERE `productId` = $productId");
                $stmt->execute();
                $row = $stmt->fetch(PDO::FETCH_ASSOC); ?>
                
                Do you want to set <?php echo($row["pBrand"]); ?> | <?php echo($row["pModel"]); ?> | <?php echo($row["pName"]); ?> as the second featured product on the landing page?
                <form action="process-set-featured2.php" method="POST">
                	<input type="hidden" name="productId" value="<?php echo($row["productId"]);?>"></br>
                	<input type="submit" value="Set featured 2" />
                </form> </br>
