	...
	<html>
        ...
        <?php include("../includes/2ndlevel-header.php"); ?>
            <main>
                ...
                <h2>Set Featured product 3</h2>
                <?php include('../includes/db-config.php');
                $stmt = $pdo->prepare("SELECT * FROM `product` WHERE `productId` = $productId");
                $stmt->execute();
                $row = $stmt->fetch(PDO::FETCH_ASSOC); ?>
                
                Do you want to set <?php echo($row["pBrand"]); ?> | <?php echo($row["pModel"]); ?> | <?php echo($row["pName"]); ?> as the third featured product on the landing page?
                <form action="process-set-featured3.php" method="POST">
                	<input type="hidden" name="productId" value="<?php echo($row["productId"]);?>"></br>
                	<input type="submit" value="Set featured 3" />
                </form> </br>
